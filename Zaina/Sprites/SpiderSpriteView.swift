...
class SpiderSpriteView: SpriteView {
  ...
  override func setup() {
    ...
    framesHolder = FramesHolder(idel: arrayIdel)
    framesHolder?.top = arrayLeft
    framesHolder?.left = arrayLeft
    framesHolder?.right = arrayRight
    framesHolder?.bottom = arrayRight
    framesHolder?.idel = arrayIdel
  }
}
